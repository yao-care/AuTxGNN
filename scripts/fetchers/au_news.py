#!/usr/bin/env python3
"""Australia health news fetcher.

Fetches health news from Australian sources using Google News RSS.
"""
...
# Australian health news sources
AU_NEWS_SOURCES = [
    "abc.net.au",
    "smh.com.au",
    "theage.com.au",
...
HEALTH_KEYWORDS_EN = [
    ...
    "cancer", "diabetes", "heart disease", "research", "TGA",
    "Therapeutic Goods", "medical", "doctor", "prescription",
]


def fetch_google_news_rss(query: str, region: str = "AU") -> list[dict]:
    """Fetch news from Google News RSS.

    Args:
        query: Search query
        region: Country code (AU for Australia)
...
def filter_health_news(items: list[dict]) -> list[dict]:
    ...
    for item in items:
        ...
        # Check for Australian sources
        link = item.get("link", "")
        is_au_source = any(src in link for src in AU_NEWS_SOURCES)

        if is_health or is_au_source:
...
def main():
    ...
    print("=" * 60)
    print("AuTxGNN - Australia Health News Fetcher")
    print("=" * 60)
    print()
