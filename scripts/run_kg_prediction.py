...
前置條件:
    1. 已下載澳洲藥品資料 (data/raw/DrugList.xml)
...
from autxgnn.data.loader import load_fda_drugs, filter_active_drugs, get_drug_summary
from autxgnn.mapping.drugbank_mapper import map_fda_drugs_to_drugbank
from autxgnn.predict.repurposing import find_repurposing_candidates


def main():
    print("=" * 60)
    print("AuTxGNN - 執行知識圖譜方法預測")
    print("=" * 60)
    print()
    ...
    # ARTG 欄位: ARTG_ID, Product_Name, Active_Ingredients
    ...
    # 3. 尋找老藥新用候選
    # 澳洲資料沒有適應症欄位，所以使用簡化方法：
    # 直接生成所有 (藥物, 疾病) 配對
    print()
    print("3. 尋找老藥新用候選...")
    print("   （澳洲資料無適應症欄位，將生成所有可能配對）")

    # 載入藥物-疾病關係 (columns: relation, x_id, x_name, y_id, y_name)
    from autxgnn.predict.repurposing import load_drug_disease_relations
